...
public class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            var descriptor = services.First(d => d.ServiceType == typeof(DbContextOptions<StickerContext>));
            services.Remove(descriptor);
            services.AddDbContext<StickerContext>(options =>
            {
    ...
    public void InitializeDatabase(Action<StickerContext> action)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<StickerContext>();
        db.Database.EnsureDeleted();
        db.Database.EnsureCreated();
        action(db);
    }

    public Tout QueryDatabase<Tout>(Func<StickerContext, Tout> query)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<StickerContext>();
        return query(db);
